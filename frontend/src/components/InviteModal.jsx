...
const InviteModal = ({ onClose, channel }) => {
  ...
  return (
    <div className="create-channel-modal-overlay">
      <div className="create-channel-moda">
        {/* HEADER */}
          ...
          {users.length > 0 &&
            users.map((user) => {
              ...
              return (
                <label
                  key={user.id}
                  className={`flex itemcen gap-4 p-3 rounded-lg cursor-pointer transition-all shadow-sm bg-white hover:bg-[#f5f3ff] border-2 ${
                    isChecked
                      ? "border-[#611f69] bg-[#f5f3ff]"
                      : "border-gray-200"
                  }`}
                >
                  <input
